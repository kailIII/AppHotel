...
public class formHabita extends JDialog {
	...
	private BuscarHabitacion buscarHabicion;
	public formHabita() {
		setTitle("Habitaci\u00F3n");
		setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
		setBounds(100, 100, 315, 309);
		...
		lblBuscar.setBounds(10, 240, 52, 23);
		...
		TfBuscar.setBounds(67, 238, 223, 30);
		...
		btnGuardar.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (TfMontoDia.getText().length() == 0) {
		    		...
		    		if (formHabita.this.btnGuardar.getText().compareTo("Guardar")==0) {
						guardarHabitacion();
						LimpiarCampos();
						deshabilitarCampos();
						habilitarCodigo();
		...
		scrollPane.setBounds(0, 0, 299, 235);
				...
				public void keyPressed(KeyEvent e) {
					if (e.getKeyCode() == KeyEvent.VK_ENTER) {
						for (int i : table.getSelectedRows()) {
					        buscarHabicion.BuscarHabitacionLista(lista.get(i));
					        System.out.println(buscarHabicion);
						}
				            dispose();
		...
		TfdescripHabitacion.requestFocus();
			...
			private void buscarHabitacionPorCodigo() {
				...
				public void consultar() {
					...
					public BuscarHabitacion getBuscarHabicion() {
						return buscarHabicion;
					}
					public void setBuscarHabicion(BuscarHabitacion buscarHabicion) {
						this.buscarHabicion = buscarHabicion;
					}
